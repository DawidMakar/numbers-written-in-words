...
class NumberToWordsViewTest(TestCase):
    ...
    def test_value_range(self):
        response = self.client.post(reverse_lazy('number_to_words'), data={'number': 1234567890987654})
        self.assertEqual(response.status_code, 200)
        self.assertTrue(getattr(response, 'context_data') and response.context_data)
...
class NumberToWordsFunctionTest(TestCase):
    ...
    def test_thousands(self):
        ...
        self.assertEqual(convert_number_to_words(-3273),
                         'minus trzy tysiące dwieście siedemdziesiąt trzy')
